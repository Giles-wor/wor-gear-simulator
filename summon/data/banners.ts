...
// 고대: 영주 0.72% × 8명 / 일반 1% × ~30명, 185→+8%/회→200픽, 영주 전용 천장.
...
const placeholderBanners: Record<BannerConfig['id'], BannerConfig> = {
  ...
  ancient: {
    ...
    placeholder: false,
    notes:
      '고대 소환: 보장 천장 2개(독립 카운터). ① 영주(알수없는자 포함) 0.72%, 185회 미획득 후 +8%/회, 200픽 확정. ② 일반 레전더리(영주·알수없는자 제외) 1%, 130회 미획득 후 +5%/회, 150픽 확정. ×20 픽업 + ×2 stacking. 보통 영주+일반 각 1명씩 픽업. (2026-06-29 풀 조정: 비주류 영주/알수없는자 7명 제외 → 영주 풀 8명. 그룹 0.72%는 유지되어 영주 1명당 확률은 상승.)',
  },
  divine: {
    id: 'divine',
