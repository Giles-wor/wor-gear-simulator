...
나간 사람이 옛 코드를 알아서 코드를 바꾸고 싶을 때: 잠금 화면의 **🔁 코드 변경(이전)** 에서
**이전 코드 + 새 코드**를 입력하면, 이전 코드 내용을 **새 코드로 복사**하고 새 코드로 자동 입장합니다.
이후 새 코드로 작업한 건 옛 코드엔 반영되지 않아 나간 사람은 옛 코드로 더 못 봅니다(옛 코드 행은 그 시점 스냅샷으로 남음).
