...
export function getBestStatRecommendation(
  ...
  const atkGain = calculateBuild(hero, leftSet, rightSet, atkBuild).itemMaxCumulative30s - current.itemMaxCumulative30s
  const cdGain = calculateBuild(hero, leftSet, rightSet, cdBuild).itemMaxCumulative30s - current.itemMaxCumulative30s
  const aspdGain = calculateBuild(hero, leftSet, rightSet, aspdBuild).itemMaxCumulative30s - current.itemMaxCumulative30s
  ...
  return {
    ...
    reason: `현재 기준 30초 누적 상승폭은 ${ranking[0].key} > ${ranking[1].key} > ${ranking[2].key} 순입니다.`
  }
}
